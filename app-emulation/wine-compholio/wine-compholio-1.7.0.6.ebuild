...
inherit autotools-multilib eutils fdo-mime flag-o-matic gnome2-utils l10n multilib pax-utils toolchain-funcs virtualx versionator
...
SRC_URI="${SRC_URI}
	...
	http://dev.gentoo.org/~tetromino/distfiles/${MY_PN}/${PULSE_PATCHES}.tar.bz2
	http://dev.gentoo.org/~tetromino/distfiles/${MY_PN}/${WINE_GENTOO}.tar.bz2
	http://dev.gentoo.org/~ryao/distfiles/${COMPHOLIO_PATCHES}.tar.bz2"
...
src_unpack() {
	...
	unpack "${COMPHOLIO_PATCHES}.tar.bz2"
...
src_prepare() {
	...
	local PATCHES=(
		...
		"${FILESDIR}"/${MY_PN}-1.5.17-osmesa-check.patch #429386
...
do_configure() {
	...
	myeconfargs+=( --libdir="/opt/wine-compholio/$(get_libdir)" )
	if use amd64; then
		if [[ ${ABI} == amd64 ]]; then
...
src_configure() {
	...
	local myeconfargs=( # common
		--datadir="/opt/wine-compholio/share"
		--datarootdir="/opt/wine-compholio/share"
		--infodir="/opt/wine-compholio/share/info"
		--localstatedir="/opt/wine-compholio/var/lib"
		--mandir="/opt/wine-compholio/share/man"
		--prefix="/opt/wine-compholio"
		--sysconfdir="/etc/wine-compholio"
		$(use_with alsa)
		$(use_with capi)
		$(use_with lcms cms)
		$(use_with cups)
...
src_install() {
	...
	if use gecko ; then
		insinto /opt/wine-compholio/share/wine/gecko
		use abi_x86_32 && doins "${DISTDIR}"/wine_gecko-${GV}-x86.msi
		use abi_x86_64 && doins "${DISTDIR}"/wine_gecko-${GV}-x86_64.msi
	fi
	if use mono ; then
		insinto /opt/wine-compholio/share/wine/mono
		doins "${DISTDIR}"/wine-mono-${MV}.msi
	fi
	if ! use perl ; then
		rm "${D}"opt/wine-compholio/bin/{wine{dump,maker},function_grep.pl} "${D}"opt/wine-compholio/share/man/man1/wine{dump,maker}.1 || die
	fi

	use abi_x86_32 && pax-mark psmr "${D}"opt/wine-compholio/bin/wine{,-preloader} #255055
	use abi_x86_64 && pax-mark psmr "${D}"opt/wine-compholio/bin/wine64{,-preloader}

	if use abi_x86_64 && ! use abi_x86_32; then
		dosym /opt/wine-compholio/bin/wine{64,} # 404331
		dosym /opt/wine-compholio/bin/wine{64,}-preloader
	fi

	# respect LINGUAS when installing man pages, #469418
	for l in de fr pl; do
		use linguas_${l} || rm -r "${D}"opt/wine-compholio/man/${l}*
	done

	# Remove menu stuff that was installed outside of /opt
	# This avoids a conflict with app-emulation/wine
	rm -r "${D}usr/share"/{applications,desktop-directories,icons}
	rm -r "${D}etc"
}
